...
model = keras.applications.DenseNet201(include_top=False, weights='imagenet',
...
def get_encodings(img):
    ...
    # Convert RGBA images to RGB (remove alpha channel)
    if img.mode == 'RGBA':
        img = img.convert('RGB')
    ...
    img_preprocessed = keras.applications.densenet.preprocess_input(
        img_array)
